...
def _is_external(href: str, origin_domain: str) -> bool:
    ...
    if not origin_domain:
        return True
    same_site = (
        link_domain == origin_domain
        or link_domain.endswith(f".{origin_domain}")
        or origin_domain.endswith(f".{link_domain}")
    )
...
def synthesize_html_features_for_url(
    url: str,
    label: int = 0,
    source: Optional[str] = None,
) -> Dict[str, float]:
    """
    Build a deterministic, realistic HTML-feature profile for URL-only datasets.

    Most public URL feeds do not include raw HTML. Filling all HTML features with
    zero teaches the model that real page structure is suspicious at inference
    time. These profiles keep the model exposed to normal legit portals
    (including login/account pages) and common credential-harvesting structure.
    """
    seed_text = f"{url}|{label}|{source or ''}"
    seed = int(hashlib.sha256(seed_text.encode("utf-8", errors="ignore")).hexdigest()[:8], 16)

    def bucket(mod: int) -> int:
        return seed % mod

    url_lower = (url or "").lower()
    auth_like = any(token in url_lower for token in (
        "login", "signin", "account", "auth", "verify", "password",
        "payment", "bank", "tax", "refund",
    ))

    features = {name: 0.0 for name in HTML_FEATURE_NAMES}

    if int(label) == 0:
        # Legitimate sites can absolutely have forms, scripts, social links,
        # contact pages, and large DOMs. Keep those patterns normal.
        login_form = int(auth_like or bucket(5) == 0)
        features.update({
            "form_action_external": 0,
            "iframe_count": bucket(3),
            "hidden_input_count": bucket(4) if login_form else bucket(2),
            "external_link_ratio": min(0.45, 0.04 + (bucket(18) / 100.0)),
            "script_count": 4 + bucket(35),
            "meta_refresh_present": 0,
            "login_form_present": login_form,
            "password_field_count": login_form,
            "title_domain_mismatch": 0,
            "favicon_external": int(bucket(7) == 0),
            "anchor_to_javascript": bucket(4),
            "copyright_year_present": 1,
            "social_media_links_count": bucket(6),
            "page_size_bytes": 18000 + bucket(140000),
            "whitespace_ratio": 0.12 + (bucket(18) / 100.0),
            "noscript_count": bucket(3),
            "internal_link_count": 8 + bucket(70),
            "image_count": 2 + bucket(30),
            "ad_network_links": bucket(4),
            "contact_page_link": 1,
        })
    else:
        credential_like = int(auth_like or bucket(3) != 0)
        features.update({
            "form_action_external": int(bucket(5) != 0),
            "iframe_count": bucket(4),
            "hidden_input_count": 1 + bucket(7) if credential_like else bucket(3),
            "external_link_ratio": 0.35 + (bucket(45) / 100.0),
            "script_count": bucket(22),
            "meta_refresh_present": int(bucket(8) == 0),
            "login_form_present": credential_like,
            "password_field_count": credential_like,
            "title_domain_mismatch": int(bucket(5) != 0),
            "favicon_external": int(bucket(4) == 0),
            "anchor_to_javascript": bucket(5),
            "copyright_year_present": int(bucket(3) == 0),
            "social_media_links_count": bucket(3),
            "page_size_bytes": 3000 + bucket(45000),
            "whitespace_ratio": 0.06 + (bucket(20) / 100.0),
            "noscript_count": bucket(2),
            "internal_link_count": bucket(7),
            "image_count": bucket(8),
            "ad_network_links": bucket(3),
            "contact_page_link": int(bucket(6) == 0),
        })
