...
def run_feature_extraction(force: bool = False):
    ...
    from src.features.html_features import (
        ...
        synthesize_html_features_for_url,
    )
    ...
    for split_name in ["train", "val", "test"]:
        ...
        if output_path.exists() and force:
            logger.info(f"Regenerating existing feature file for {split_name}: {output_path}")
        ...
        # 3. URL-only feeds still need realistic HTML distributions for
        # training; otherwise live page structure becomes a false-positive
        # signal. Mendeley rows with real HTML override these profiles below.
        synthetic_html = pd.DataFrame([
            synthesize_html_features_for_url(row["url"], row["label"], row.get("source"))
            for _, row in df.iterrows()
        ], columns=HTML_FEATURE_NAMES)
        result = pd.concat([result.reset_index(drop=True), synthetic_html.reset_index(drop=True)], axis=1)

        # 4. Join with the pre-extracted HTML structural features
        if mendeley_meta is not None and html_features_path.exists():
            ...
            # Prefer actual HTML features where present; keep synthetic profiles
            # for URL-only feeds.
            for col in HTML_FEATURE_NAMES:
                actual_col = f"{col}_actual"
                if actual_col in result.columns:
                    result[col] = result[actual_col].combine_first(result[col])
                    result.drop(columns=[actual_col], inplace=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="Regenerate split feature CSVs even when they already exist.",
    )
    args = parser.parse_args()
    run_feature_extraction(force=args.force)
