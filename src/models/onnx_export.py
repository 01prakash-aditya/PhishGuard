...
def export_lgb_to_onnx(model_path: Path, output_name: str = "phishguard_edge.onnx"):
    ...
    # Strip ZipMap node (known to cause WASM bridge serialization crashes in ONNX Web)
    try:
        zipmap_nodes = [n for n in onnx_model.graph.node if n.op_type == 'ZipMap']
        if zipmap_nodes:
            zipmap_node = zipmap_nodes[0]
            onnx_model.graph.node.remove(zipmap_node)
            for node in onnx_model.graph.node:
                for i, out in enumerate(node.output):
                    if out == 'lgbmprobabilities':
                        node.output[i] = 'probabilities'
            new_out = onnx.helper.make_tensor_value_info('probabilities', onnx.TensorProto.FLOAT, [None, 2])
            for out in onnx_model.graph.output:
                if out.name == 'probabilities':
                    out.CopyFrom(new_out)
            logger.info("Stripped ZipMap node from ONNX graph for Chrome extension compatibility.")
    except Exception as e:
        logger.warning(f"Could not strip ZipMap: {e}")
